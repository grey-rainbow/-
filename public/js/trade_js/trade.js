...
function check_trade_form() {
    $.ajax({ url: "/fund_account", type: "POST", data: JSON.stringify({ username: get_current_username() }), contentType: "application/json;charset=utf-8", success: function success(data) {
...
function check_confirm_form() {
    $("#confirm_form").validate({
        ...
        submitHandler: function submitHandler(form) {
            ...
            $.ajax({ url: "/account_user_login", type: "POST", data: JSON.stringify(send_data), contentType: "application/json;charset=utf-8", success: function success(data) {
                    if (data.state == "true") {
...
function send_trade(form) {
    ...
    $.ajax({
        ...
        success: function success(data) {
            if (data.state == "true") {
                $("#msg_slot").html(create_success_alert("交易指令发布成功!<strong>该笔交易的交易号为：{}</strong>".replace("{}", data.transaction_id)));
                $("#trade-form")[0].reset();
            } else {
...
function load_transactions() {
    $.get("/all_transaction", function (data) {
        if (data.state == "true") render_transaction(data);else {
...
function create_trade_cell(id, item) {
    ...
    var formated_date = date.getFullYear() + "年" + (date.getMonth() + 1) + "月" + date.getDate() + "日" + date.getHours() + "时" + date.getMinutes() + "分" + date.getSeconds() + "秒";
...
function send_cancel() {
    ...
    $.ajax({ url: "/transaction_state", type: "POST", data: JSON.stringify(send_data), contentType: "application/json;charset=utf-8", success: function success(data) {
            if (data.state == "false") {
