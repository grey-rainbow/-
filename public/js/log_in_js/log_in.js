...
function time() {
    var time_div = document.getElementById('showtime');
    var now = new Date();
...
function change() {
    ...
    if (newpswd1 !== newpswd2) {
        ...
        var info = { 'username': sessionStorage.getItem("log_security_username"), 'password': oldpswd, 'new_password': newpswd1 };
        var text = JSON.stringify(info);
        ...
        xmlhttp.onreadystatechange = function () {
            if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
                ...
                var temp = JSON.parse(xmlhttp.responseText);
                alert(temp.msg);
                if (temp.state == "true") window.location.href = "/index";
                elselk22;
